...
public class BaseDaoImp implements BaseDao {
	...
	public <T> QueryResult<T> getQueryResult(Class<T> entityClass,
			int startIndex, int MaxResults, Object[] parameters, String where,
		...
		Query query = session.createQuery("select o from "
				...
				+ " o "
				+ (where == null || "".equals(where.trim()) ? "" : "where "
						+ where + " ") + buildOrderBy(orderBy));
		...
		query = session.createQuery("select count(o) from "
				+ getEntityName(entityClass)
				+ " o "
				+ (where == null || "".equals(where.trim()) ? "" : "where "
						+ where + ""));
	...
	public <T> QueryResult<T> getQueryResult(Class<T> entityClass,
			int startIndex, int MaxResults, Object[] parameters, String where) {
		return this.getQueryResult(entityClass, startIndex, MaxResults,
	...
	public <T> QueryResult<T> getQueryResult(Class<T> entityClass,
		...
		return this.getQueryResult(entityClass, startIndex, MaxResults, null,
				null, orderBy);
	}
	...
	public <T> QueryResult<T> getQueryResult(Class<T> entityClass,
		...
		return this.getQueryResult(entityClass, -1, -1, parameters, where,
				orderBy);
	}
	...
	public <T> QueryResult<T> getQueryResult(Class<T> entityClass,
		...
		return this
				.getQueryResult(entityClass, -1, -1, parameters, where, null);
	}
	...
	public String buildOrderBy(LinkedHashMap<String, String> orderBy) {
		...
		if (orderBy != null && orderBy.size() > 0) {
			...
			for (String key : orderBy.keySet()) {
				sb.append("o." + key + " ").append(orderBy.get(key))
						.append(",");
			}
